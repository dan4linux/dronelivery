...
public class DeliveryProcessorTest {
	...
	public void runTest() throws IOException {
		...
		new DeliveryProcessor("target/test-classes/test.schedule", new PrintStream(outContent)).run();
	...
	public void runWithBadData() {
		...
		try {
			new DeliveryProcessor("target/test-classes/bad.schedule", new PrintStream(outContent)).run();
		} catch(Throwable t) {
	...
	public void runWithMissingFile() {
		...
		boolean caughtException = false;
		try {
			new DeliveryProcessor("target/test-classes/nofile.schedule", new PrintStream(outContent)).run();
		} catch(Throwable t) {
			caughtException = true;
		}
